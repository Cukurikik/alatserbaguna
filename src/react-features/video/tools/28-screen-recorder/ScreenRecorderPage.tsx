...
export const ScreenRecorderPage: React.FC = () => {
  const [isRecording, setIsRecording] = useState(false);
  const [recordedUrl, setRecordedUrl] = useState<string | null>(null);
  const [options, setOptions] = useState({ audio: true, frameRate: 30 });
        ...
        <div className="lg:col-span-7 space-y-8">
          <div className="flex items-center gap-4 mb-8">
            ...
            <div>
              ...
              <p className="text-text-muted">Record your screen or specific windows directly in the browser.</p>
            </div>
          </div>
          ...
          <div className="aspect-video bg-white/5 rounded-3xl border border-white/10 overflow-hidden relative group">
            {isRecording ? (
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                ...
                <span className="text-text-muted">Ready to record</span>
              </div>
            )}
          ...
          <div className="bg-white/5 rounded-3xl p-8 border border-white/10 space-y-8">
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between p-4 bg-white/5 rounded-2xl border border-white/10">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-[10px] text-text-muted">Include system/mic audio</div>
                  </div>
                </div>
                <button
              ...
              <div className="space-y-3">
                <label className="text-sm text-text-muted">Frame Rate</label>
                <div className="grid grid-cols-3 gap-3">
                  {[24, 30, 60].map((fps) => (
                    <button
                      key={fps}
                      onClick={() => setOptions(prev => ({ ...prev, frameRate: fps }))}
                      className={`
