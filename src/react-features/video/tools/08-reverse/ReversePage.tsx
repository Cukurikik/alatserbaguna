...
import { ReverseEngine } from './ReverseEngine';
...
const engine = new ReverseEngine();
    ...
    <div className="min-h-screen bg-[#0a0a0f] text-white p-4 md:p-8 font-sans">
      <div className="max-w-6xl mx-auto">
        <header className="mb-8">
          ...
          <p className="text-slate-400 font-medium">Rewind reality. Play your videos backward with one click.</p>
        </header>
            ...
            <div className="relative aspect-video bg-black/40 rounded-3xl border border-white/5 overflow-hidden group flex items-center justify-center">
              {previewUrl ? (
                ...
                <label className="flex flex-col items-center gap-4 cursor-pointer group-hover:scale-105 transition-transform duration-500">
                  <div className="w-20 h-20 rounded-2xl bg-white/5 flex items-center justify-center border border-white/10 group-hover:border-pink-500/50 group-hover:bg-pink-500/10 transition-all">
                    <Upload className="w-10 h-10 text-slate-400 group-hover:text-pink-400" />
                  </div>
            ...
            {currentTask && currentTask.status === 'processing' && (
              <div className="bg-white/5 border border-white/5 rounded-3xl p-6 space-y-4">
                <div className="flex justify-between items-center">
                  <span className="text-sm font-bold text-pink-400 uppercase tracking-widest">Reversing Reality...</span>
          ...
          <div className="space-y-6">
            <div className="bg-white/5 border border-white/5 rounded-3xl p-6 space-y-8">
              <div className="space-y-4">
                <h3 className="text-xs font-black text-slate-500 uppercase tracking-[0.2em]">Audio Options</h3>
                <button 
                  onClick={() => setOptions(prev => ({ ...prev, reverseAudio: !prev.reverseAudio }))}
                  className={`w-full flex items-center justify-between p-4 rounded-2xl border transition-all ${options.reverseAudio ? 'bg-pink-500/10 border-pink-500/50 text-pink-400' : 'bg-white/5 border-white/5 text-slate-400'}`}
                >
                  <div className="flex items-center gap-3">
                    {options.reverseAudio ? <Volume2 className="w-5 h-5" /> : <VolumeX className="w-5 h-5" />}
                    <span className="text-sm font-bold">Reverse Audio</span>
                  </div>
                  <div className={`w-10 h-5 rounded-full relative transition-colors ${options.reverseAudio ? 'bg-pink-500' : 'bg-white/10'}`}>
                    <div className={`absolute top-1 w-3 h-3 rounded-full bg-white transition-all ${options.reverseAudio ? 'left-6' : 'left-1'}`} />
                  </div>
                </button>
              </div>
