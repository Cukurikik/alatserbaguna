...
export default function LoopPage() {
  ...
  const [options, setOptions] = useState<LoopOptions>({
    loopCount: 2,
    outputFormat: 'mp4'
  });
    ...
    <div className="min-h-screen bg-[#0a0a0f] text-white p-4 md:p-8 font-sans">
      <div className="max-w-6xl mx-auto">
        <header className="mb-8">
          <h1 className="text-4xl font-black tracking-tighter mb-2 bg-gradient-to-r from-indigo-400 to-blue-500 bg-clip-text text-transparent">
            LOOP ENGINE
          </h1>
          <p className="text-slate-400 font-medium">Extend your moments. Create seamless loops with zero quality loss.</p>
        </header>
            ...
            <div className="relative aspect-video bg-black/40 rounded-3xl border border-white/5 overflow-hidden group flex items-center justify-center">
              {previewUrl ? (
                ...
                <label className="flex flex-col items-center gap-4 cursor-pointer group-hover:scale-105 transition-transform duration-500">
                  <div className="w-20 h-20 rounded-2xl bg-white/5 flex items-center justify-center border border-white/10 group-hover:border-indigo-500/50 group-hover:bg-indigo-500/10 transition-all">
            ...
            {currentTask && currentTask.status === 'processing' && (
              <div className="bg-white/5 border border-white/5 rounded-3xl p-6 space-y-4">
                <div className="flex justify-between items-center">
                  <span className="text-sm font-bold text-indigo-400 uppercase tracking-widest">Generating Loop...</span>
            ...
            <div className="bg-white/5 border border-white/5 rounded-3xl p-6 space-y-8">
              <div className="space-y-4">
                <div className="flex justify-between items-end">
                  <h3 className="text-xs font-black text-slate-500 uppercase tracking-[0.2em]">Loop Count</h3>
                  <span className="text-2xl font-black text-indigo-400">{options.loopCount}x</span>
                </div>
                ...
                <input 
                  ...
                  onChange={(e) => setOptions(prev => ({ ...prev, loopCount: parseInt(e.target.value) }))}
                ...
                <div className="grid grid-cols-4 gap-2">
                  {[2, 3, 5, 10].map(count => (
                    <button
                      key={count}
                      onClick={() => setOptions(prev => ({ ...prev, loopCount: count }))}
                      className={`py-2 rounded-xl text-[10px] font-black border transition-all ${options.loopCount === count ? 'bg-indigo-500 border-indigo-500 text-black' : 'bg-white/5 border-white/5 text-slate-400 hover:bg-white/10'}`}
                    >
                      {count}x
                    </button>
                  ))}
              ...
              {currentTask?.status === 'success' && (
                ...
                >
                  ...
                  <a 
                    ...
                    download={`looped_${options.loopCount}x_${file?.name}`}
