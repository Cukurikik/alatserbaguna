...
import { RotateFlipEngine } from './RotateFlipEngine';
...
const engine = new RotateFlipEngine();

export default function RotateFlipPage() {
  const [file, setFile] = useState<File | null>(null);
  const [previewUrl, setPreviewUrl] = useState<string | null>(null);
  const [options, setOptions] = useState<RotateFlipOptions>({
    degrees: 0,
    flipH: false,
    flipV: false,
    outputFormat: 'mp4'
  ...
  const toggleRotate = () => {
    setOptions(prev => ({
      ...prev,
      degrees: ((prev.degrees + 90) % 360) as 0 | 90 | 180 | 270
    }));
  };

  const toggleFlipH = () => setOptions(prev => ({ ...prev, flipH: !prev.flipH }));
  const toggleFlipV = () => setOptions(prev => ({ ...prev, flipV: !prev.flipV }));
    ...
    <div className="min-h-screen bg-[#0a0a0f] text-white p-4 md:p-8 font-sans">
      <div className="max-w-6xl mx-auto">
        <header className="mb-8">
          ...
          <p className="text-slate-400 font-medium">Adjust video orientation and mirroring with precision.</p>
        </header>
            ...
            <div className="relative aspect-video bg-black/40 rounded-3xl border border-white/5 overflow-hidden group flex items-center justify-center">
              {previewUrl ? (
                <div 
                  ...
                  style={{
                    transform: `rotate(${options.degrees}deg) scaleX(${options.flipH ? -1 : 1}) scaleY(${options.flipV ? -1 : 1})`
                  }}
                >
                  <video 
                    ref={videoRef}
                ...
                <label className="flex flex-col items-center gap-4 cursor-pointer group-hover:scale-105 transition-transform duration-500">
                  <div className="w-20 h-20 rounded-2xl bg-white/5 flex items-center justify-center border border-white/10 group-hover:border-cyan-500/50 group-hover:bg-cyan-500/10 transition-all">
            ...
            {currentTask && currentTask.status === 'processing' && (
              <div className="bg-white/5 border border-white/5 rounded-3xl p-6 space-y-4">
                <div className="flex justify-between items-center">
                  <span className="text-sm font-bold text-cyan-400 uppercase tracking-widest">Processing Video...</span>
          ...
          <div className="space-y-6">
            <div className="bg-white/5 border border-white/5 rounded-3xl p-6 space-y-8">
              <div className="space-y-4">
                <h3 className="text-xs font-black text-slate-500 uppercase tracking-[0.2em]">Orientation</h3>
                <div className="grid grid-cols-3 gap-3">
                  <button 
                    onClick={toggleRotate}
                    className="flex flex-col items-center gap-2 p-4 rounded-2xl bg-white/5 border border-white/5 hover:bg-white/10 hover:border-cyan-500/30 transition-all group"
                  >
                    <RotateCw className="w-6 h-6 text-slate-400 group-hover:text-cyan-400 group-hover:rotate-90 transition-all duration-500" />
                    <span className="text-[10px] font-bold uppercase tracking-widest">Rotate</span>
                  </button>
                  <button 
                    onClick={toggleFlipH}
                    className={`flex flex-col items-center gap-2 p-4 rounded-2xl border transition-all group ${options.flipH ? 'bg-cyan-500/10 border-cyan-500/50 text-cyan-400' : 'bg-white/5 border-white/5 text-slate-400 hover:bg-white/10'}`}
                  >
                    <FlipHorizontal className="w-6 h-6 group-hover:scale-x-[-1] transition-transform duration-500" />
                    <span className="text-[10px] font-bold uppercase tracking-widest">Flip H</span>
                  </button>
                  <button 
                    onClick={toggleFlipV}
                    className={`flex flex-col items-center gap-2 p-4 rounded-2xl border transition-all group ${options.flipV ? 'bg-purple-500/10 border-purple-500/50 text-purple-400' : 'bg-white/5 border-white/5 text-slate-400 hover:bg-white/10'}`}
                  >
                    <FlipVertical className="w-6 h-6 group-hover:scale-y-[-1] transition-transform duration-500" />
                    <span className="text-[10px] font-bold uppercase tracking-widest">Flip V</span>
                  </button>
                </div>
              </div>
            ...
            <AnimatePresence>
              ...
              {currentTask?.status === 'error' && (
                <motion.div 
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: 1, y: 0 }}
                  className="bg-red-500/10 border border-red-500/20 rounded-3xl p-6 space-y-2"
                >
                  <div className="flex items-center gap-3 text-red-400">
                    <AlertCircle className="w-6 h-6" />
                    <span className="font-bold">Error Occurred</span>
                  </div>
                  <p className="text-xs text-red-400/80 font-medium">{currentTask.error}</p>
                </motion.div>
              )}
            </AnimatePresence>
          </div>
