...
import { SpeedControlEngine } from './SpeedControlEngine';
...
const engine = new SpeedControlEngine();

export default function SpeedControlPage() {
  const [file, setFile] = useState<File | null>(null);
  const [previewUrl, setPreviewUrl] = useState<string | null>(null);
  const [options, setOptions] = useState<SpeedControlOptions>({
    multiplier: 1.0,
    preservePitch: true,
    outputFormat: 'mp4'
  });
  ...
  const handleProcess = async () => {
    ...
    startTask({
      id: taskId,
      name: `Speed Control: ${options.multiplier}x`,
      status: 'processing',
      progress: 0,
    ...
    <div className="min-h-screen bg-[#0a0a0f] text-white p-4 md:p-8 font-sans">
      <div className="max-w-6xl mx-auto">
        <header className="mb-8">
          <h1 className="text-4xl font-black tracking-tighter mb-2 bg-gradient-to-r from-yellow-400 to-orange-500 bg-clip-text text-transparent">
            SPEED CONTROL
          </h1>
          <p className="text-slate-400 font-medium">Manipulate time. Fast forward or slow down with high fidelity.</p>
        </header>
            ...
            <div className="relative aspect-video bg-black/40 rounded-3xl border border-white/5 overflow-hidden group flex items-center justify-center">
              {previewUrl ? (
                <video 
                  ...
                  playbackRate={options.multiplier}
                />
              ) : (
                <label className="flex flex-col items-center gap-4 cursor-pointer group-hover:scale-105 transition-transform duration-500">
                  <div className="w-20 h-20 rounded-2xl bg-white/5 flex items-center justify-center border border-white/10 group-hover:border-yellow-500/50 group-hover:bg-yellow-500/10 transition-all">
                    <Upload className="w-10 h-10 text-slate-400 group-hover:text-yellow-400" />
                  </div>
            ...
            {currentTask && currentTask.status === 'processing' && (
              <div className="bg-white/5 border border-white/5 rounded-3xl p-6 space-y-4">
                <div className="flex justify-between items-center">
                  <span className="text-sm font-bold text-yellow-400 uppercase tracking-widest">Warping Time...</span>
            ...
            <div className="bg-white/5 border border-white/5 rounded-3xl p-6 space-y-8">
              <div className="space-y-4">
                <div className="flex justify-between items-end">
                  <h3 className="text-xs font-black text-slate-500 uppercase tracking-[0.2em]">Speed Multiplier</h3>
                  <span className="text-2xl font-black text-yellow-400">{options.multiplier}x</span>
                </div>
                ...
                <input 
                  ...
                  onChange={(e) => setOptions(prev => ({ ...prev, multiplier: parseFloat(e.target.value) }))}
                ...
                <div className="grid grid-cols-4 gap-2">
                  {speedPresets.map(preset => (
                    <button
                      key={preset}
                      onClick={() => setOptions(prev => ({ ...prev, multiplier: preset }))}
                      className={`py-2 rounded-xl text-[10px] font-black border transition-all ${options.multiplier === preset ? 'bg-yellow-500 border-yellow-500 text-black' : 'bg-white/5 border-white/5 text-slate-400 hover:bg-white/10'}`}
                    >
                      {preset}x
                    </button>
                  ))}
              ...
              {currentTask?.status === 'success' && (
                ...
                >
                  ...
                  <a 
                    ...
                    download={`speed_${options.multiplier}x_${file?.name}`}
