...
export class SpeedService {
  changeSpeed(file: File, format: ExportFormat, speed: number, pitchLock: boolean): Observable<WorkerMessage<{ blob: Blob, sizeMB: number }>> {
    return new Observable(observer => {
      let isCancelled = false;
      const worker = new Worker(new URL('./speed.worker', import.meta.url), { type: 'module' });
      ...
      worker.postMessage({ file, format, speed, pitchLock });
