...
import { WatermarkActions, WatermarkState, selectWatermarkState } from './watermark.store';
    ...
    <div class="h-full w-full bg-gray-950/40 backdrop-blur-xl border border-gray-800/50 rounded-2xl p-6 flex flex-col overflow-y-auto custom-scrollbar" [@fadeIn]>
      ...
      <div class="mb-8 flex justify-between items-start">
        <div>
          ...
          <p class="text-gray-400 text-sm mt-1 font-medium italic opacity-80 uppercase tracking-widest">Embed inaudible digital watermarks for copyright protection.</p>
        </div>
          ...
          <div class="flex-1 flex flex-col lg:flex-row gap-8 min-h-0" [@fadeIn]>
            ...
            <div class="w-full lg:w-80 flex flex-col gap-6">
              ...
              <div class="bg-gray-900/20 rounded-2xl p-6 border border-white/5 border-dashed">
                ...
                <p class="text-xs text-gray-600 leading-relaxed">Embed inaudible digital watermarks for copyright protection.</p>
...
export class WatermarkComponent implements OnDestroy {
  ...
  private service = inject(WatermarkService);
  ...
  onFileSelected(files: File[]): void {
    if (files.length > 0) {
      this.store.dispatch(WatermarkActions.loadFile({ file: files[0] }));
    }
  }
  ...
  onProcess(state: WatermarkState): void {
    ...
    const interval = setInterval(() => {
      ...
      if (progress >= 100) {
        ...
        this.store.dispatch(WatermarkActions.processingSuccess({ outputBlob: blob, outputSizeMB: blob.size / 1024 / 1024 }));
      } else {
        this.store.dispatch(WatermarkActions.updateProgress({ value: Math.round(progress) }));
      }
    }, 300);
  }
