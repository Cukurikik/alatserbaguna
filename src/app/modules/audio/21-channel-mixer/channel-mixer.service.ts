...
export class ChannelMixerService {
  mixChannels(file: File, format: ExportFormat, operation: ChannelOperation, monoMode: 'average' | 'left' | 'right'): Observable<WorkerMessage<{ blob: Blob, sizeMB: number }>> {
    return new Observable(observer => {
      let isCancelled = false;
      const worker = new Worker(new URL('./channel-mixer.worker', import.meta.url), { type: 'module' });
      ...
      worker.postMessage({ file, format, operation, monoMode });
