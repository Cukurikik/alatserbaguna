...
// Time Stretcher Worker — processes audio in background thread
...
self.onmessage = async (event: MessageEvent) => {
  ...
  try {
    ...
    // TODO: Implement time-stretch audio processing logic here
    // For now: pass through using FFmpeg via postMessage
    self.postMessage({ type: 'log', message: '[time-stretch] Worker started, processing...' });
