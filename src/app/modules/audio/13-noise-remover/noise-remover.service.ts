...
export class NoiseRemoverService {
  applyNoiseRemoval(
    file: File,
    format: ExportFormat,
    strength: number,
    ...
    return new Observable(observer => {
      ...
      const worker = new Worker(new URL('./noise-remover.worker', import.meta.url), { type: 'module' });
      ...
      worker.postMessage({ file, format, strength, noiseFloor });
