...
import { SilenceRemoverActions, SilenceRemoverState, selectSilenceRemoverState } from './silence-remover.store';
  ...
  template: `
    <div class="h-full w-full bg-gray-950/40 backdrop-blur-xl border border-gray-800/50 rounded-2xl p-6 flex flex-col overflow-y-auto custom-scrollbar" [@fadeIn]>
      ...
      <div class="mb-8 flex justify-between items-start">
        <div>
          <h2 class="text-4xl font-black bg-gradient-to-r from-yellow-400 via-yellow-500 to-indigo-400 bg-clip-text text-transparent drop-shadow-lg pb-1 tracking-tight">
            ✂️ Silence Remover
          </h2>
          <p class="text-gray-400 text-sm mt-1 font-medium italic opacity-80 uppercase tracking-widest">Automatically detect and remove silent regions from audio files.</p>
        </div>
        ...
        @if (!state.inputFile) {
          <div class="flex-1 flex flex-col justify-center max-w-4xl mx-auto w-full py-12" [@slideUp]>
            ...
            <div class="mt-10 grid grid-cols-1 md:grid-cols-3 gap-6">
              <div class="p-6 rounded-2xl bg-gray-900/40 border border-gray-800 backdrop-blur-md group hover:border-yellow-500/30 transition-all duration-500">
                <div class="w-10 h-10 rounded-xl bg-yellow-500/10 flex items-center justify-center text-yellow-400 mb-4 group-hover:scale-110 transition-transform">
          ...
          <div class="flex-1 flex flex-col lg:flex-row gap-8 min-h-0" [@fadeIn]>
            <div class="flex-1 flex flex-col gap-6 min-h-0">
              ...
              <div class="bg-gray-900/40 backdrop-blur-md rounded-2xl p-6 border border-gray-800">
                <div class="flex items-center gap-4">
                  <div class="w-12 h-12 rounded-xl bg-yellow-500/10 flex items-center justify-center text-yellow-400 text-2xl">🎵</div>
                  ...
                  <div class="px-3 py-1 rounded-lg bg-yellow-500/10 border border-yellow-500/20">
                    <span class="text-xs font-bold text-yellow-400 uppercase">{{ state.status }}</span>
                  </div>
                </div>
              </div>
              ...
              <div class="bg-gray-900/40 backdrop-blur-md rounded-2xl p-6 border border-gray-800">
                  ...
                  <div class="flex flex-wrap gap-2">
                    @for (fmt of formats; track fmt) {
                      <button
                        ...
                        [class]="selectedFormat === fmt ? 'bg-yellow-500/20 border-yellow-500/50 text-yellow-400' : 'bg-gray-800/50 border-gray-700 text-gray-500 hover:border-gray-600'">
                        {{ fmt.toUpperCase() }}
                      </button>
                    }
                  </div>
                ...
                <button
                  ...
                  [class]="(state.status === 'processing' || state.status === 'loading') ? 'bg-gray-800 text-gray-600 cursor-not-allowed' : 'bg-gradient-to-r from-yellow-600 to-yellow-500 hover:opacity-90 text-white shadow-lg shadow-yellow-500/20 active:scale-95'">
              ...
              @if (state.status === 'processing') {
                <div class="bg-gray-900/40 backdrop-blur-md rounded-2xl p-6 border border-gray-800 flex flex-col items-center gap-4" [@fadeIn]>
                  <app-audio-progress-ring [progress]="state.progress" [color]="'yellow'"></app-audio-progress-ring>
                  <span class="text-yellow-400 font-mono text-xs uppercase tracking-widest animate-pulse">Processing... {{ state.progress }}%</span>
                </div>
              }
              ...
              @if (state.status === 'error' && state.errorMessage) {
                <div class="p-5 bg-rose-500/10 border border-rose-500/20 rounded-2xl flex items-start gap-4" [@fadeIn]>
                  ...
                  <div class="flex-1">
                    ...
                    @if (state.retryable) {
                      <button (click)="onProcess(state)" class="mt-2 text-xs font-black text-yellow-400 hover:text-white underline underline-offset-4">Retry</button>
                    }
                  </div>
                </div>
              }
            ...
            <div class="w-full lg:w-80 flex flex-col gap-6">
              ...
              <div class="bg-gray-900/20 rounded-2xl p-6 border border-white/5 border-dashed">
                ...
                <p class="text-xs text-gray-600 leading-relaxed">Automatically detect and remove silent regions from audio files.</p>
                <div class="mt-4 space-y-2">
                  <div class="flex justify-between">
                    <span class="text-xs text-gray-600">Engine</span>
                    <span class="text-xs text-yellow-400 font-bold">FFmpeg WASM</span>
                  </div>
                  <div class="flex justify-between">
...
export class SilenceRemoverComponent implements OnDestroy {
  ...
  private service = inject(SilenceRemoverService);
  ...
  onProcess(state: SilenceRemoverState): void {
    ...
    const interval = setInterval(() => {
      ...
      if (progress >= 100) {
        ...
        this.store.dispatch(SilenceRemoverActions.processingSuccess({ outputBlob: blob, outputSizeMB: blob.size / 1024 / 1024 }));
      } else {
        this.store.dispatch(SilenceRemoverActions.updateProgress({ value: Math.round(progress) }));
      }
    }, 300);
  }

  onDownload(state: SilenceRemoverState): void {
