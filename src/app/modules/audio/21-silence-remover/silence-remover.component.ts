    ...
    <div class="h-full w-full bg-gray-950/40 backdrop-blur-xl border border-gray-800/50 rounded-2xl p-6 flex flex-col overflow-y-auto" [@fadeIn]>
      <div class="mb-8 flex justify-between items-start">
        <div>
          ...
          <p class="text-gray-400 text-sm mt-1 font-medium italic opacity-80 uppercase tracking-widest">Auto-detect and remove silent gaps from audio files.</p>
        </div>
          ...
          <div class="flex-1 flex flex-col lg:flex-row gap-8 min-h-0" [@fadeIn]>
            ...
            <div class="w-full lg:w-80 flex flex-col gap-6">
              ...
              <div class="bg-gray-900/20 rounded-2xl p-6 border border-white/5 border-dashed">
                ...
                <p class="text-xs text-gray-600 leading-relaxed">Auto-detect and remove silent gaps from audio files.</p>
...
export class SilenceRemoverComponent implements OnDestroy {
  ...
  onFileSelected(files: File[]): void {
    if (files.length > 0) this.store.dispatch(SilenceRemoverActions.loadFile({ file: files[0] }));
  }
  onProcess(state: any): void {
    if (!state.inputFile || state.status === 'processing' || state.status === 'loading') return;
    this.store.dispatch(SilenceRemoverActions.startProcessing({ format: this.selectedFormat as any }));
  }
  onDownload(state: any): void {
    if (!state.outputBlob) return;
    const url = URL.createObjectURL(state.outputBlob);
    const a = Object.assign(document.createElement('a'), {
      href: url, download: `omni_silence-remover_${state.inputFile?.name?.replace(/\.[^.]+$/, '') ?? 'audio'}.${this.selectedFormat}`
    });
