            ...
            <div class="flex-1 flex flex-col gap-6 min-h-0">
              <div class="bg-gray-900/40 backdrop-blur-md rounded-2xl p-6 border border-gray-800">
                <div class="flex items-center gap-4">
                  <div class="w-12 h-12 rounded-xl bg-violet-500/10 flex items-center justify-center text-violet-400 text-2xl">↔️</div>
...
export class StereoWidenerComponent implements OnDestroy {
  ...
  onFileSelected(files: File[]): void {
    if (files.length > 0) this.store.dispatch(StereoWidenerActions.loadFile({ file: files[0] }));
  }
  onProcess(state: any): void {
    if (!state.inputFile || state.status === 'processing' || state.status === 'loading') return;
    this.store.dispatch(StereoWidenerActions.startProcessing({ format: this.selectedFormat as any }));
  }
  onDownload(state: any): void {
    if (!state.outputBlob) return;
    const url = URL.createObjectURL(state.outputBlob);
    const a = Object.assign(document.createElement('a'), {
      href: url, download: `omni_stereo-widener_${state.inputFile?.name?.replace(/\.[^.]+$/, '') ?? 'audio'}.${this.selectedFormat}`
    });
