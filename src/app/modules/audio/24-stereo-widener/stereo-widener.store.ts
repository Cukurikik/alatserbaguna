...
export const stereowidenerReducer = createReducer(
  initialState,
  on(StereoWidenerActions.loadFile, (state, { file }) => ({ ...state, inputFile: file, status: 'loading' as ProcessingStatus, outputBlob: null, errorCode: null, errorMessage: null })),
  on(StereoWidenerActions.startProcessing, (state) => ({ ...state, status: 'processing' as ProcessingStatus, progress: 0 })),
...
export const selectStereoWidenerIsLoading = createSelector(selectStereoWidenerStatus, (s) => s === 'loading' || s === 'processing' || s === 'rendering');
export const selectStereoWidenerIsDone = createSelector(selectStereoWidenerStatus, (s) => s === 'done');
export const selectStereoWidenerHasError = createSelector(selectStereoWidenerStatus, (s) => s === 'error');
export const selectStereoWidenerCanProcess = createSelector(selectStereoWidenerState, (s) => s.inputFile !== null && s.status === 'idle');
