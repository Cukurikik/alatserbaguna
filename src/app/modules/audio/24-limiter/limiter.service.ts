...
export class LimiterService {
  applyLimiter(file: File, format: ExportFormat, ceiling: number, lookaheadMs: number, release: number, targetLUFS: number | null, truePeak: boolean): Observable<WorkerMessage<{ blob: Blob, sizeMB: number }>> {
    return new Observable(observer => {
      let isCancelled = false;
      const worker = new Worker(new URL('./limiter.worker', import.meta.url), { type: 'module' });
      ...
      worker.postMessage({ file, format, ceiling, lookaheadMs, release, targetLUFS, truePeak });
