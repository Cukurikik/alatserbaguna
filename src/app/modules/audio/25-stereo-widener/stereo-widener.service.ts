...
export class StereoWidenerService {
  applyWidening(file: File, format: ExportFormat, width: number, mode: 'stereotools' | 'extrastereo'): Observable<WorkerMessage<{ blob: Blob, sizeMB: number }>> {
    return new Observable(observer => {
      let isCancelled = false;
      const worker = new Worker(new URL('./stereo-widener.worker', import.meta.url), { type: 'module' });
      ...
      worker.postMessage({ file, format, width, mode });
