...
// Metadata Editor Worker — processes audio in background thread
...
self.onmessage = async (event: MessageEvent) => {
  ...
  try {
    ...
    // TODO: Implement metadata audio processing logic here
    // For now: pass through using FFmpeg via postMessage
    self.postMessage({ type: 'log', message: '[metadata] Worker started, processing...' });
