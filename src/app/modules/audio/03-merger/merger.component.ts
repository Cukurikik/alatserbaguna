...
import { MergerActions, MergerState, selectMergerState } from './merger.store';
...
export class MergerComponent implements OnDestroy {
  ...
  private service = inject(MergerService);
  ...
  onProcess(state: MergerState): void {
    ...
    const interval = setInterval(() => {
      ...
      if (progress >= 100) {
        ...
        this.store.dispatch(MergerActions.processingSuccess({ outputBlob: blob, outputSizeMB: blob.size / 1024 / 1024 }));
      } else {
        this.store.dispatch(MergerActions.updateProgress({ value: Math.round(progress) }));
      }
    }, 300);
  }
