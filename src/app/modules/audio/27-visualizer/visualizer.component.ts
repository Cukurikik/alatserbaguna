...
import { VisualizerActions, VisualizerState, selectVisualizerState } from './visualizer.store';
    ...
    <div class="h-full w-full bg-gray-950/40 backdrop-blur-xl border border-gray-800/50 rounded-2xl p-6 flex flex-col overflow-y-auto custom-scrollbar" [@fadeIn]>
      ...
      <div class="mb-8 flex justify-between items-start">
        <div>
          ...
          <p class="text-gray-400 text-sm mt-1 font-medium italic opacity-80 uppercase tracking-widest">Generate video with animated audio spectrum visualization.</p>
        </div>
        ...
        @if (!state.inputFile) {
          <div class="flex-1 flex flex-col justify-center max-w-4xl mx-auto w-full py-12" [@slideUp]>
            ...
            <div class="mt-10 grid grid-cols-1 md:grid-cols-3 gap-6">
              <div class="p-6 rounded-2xl bg-gray-900/40 border border-gray-800 backdrop-blur-md group hover:border-violet-500/30 transition-all duration-500">
                <div class="w-10 h-10 rounded-xl bg-violet-500/10 flex items-center justify-center text-violet-400 mb-4 group-hover:scale-110 transition-transform">
          ...
          <div class="flex-1 flex flex-col lg:flex-row gap-8 min-h-0" [@fadeIn]>
              ...
              @if (state.status === 'error' && state.errorMessage) {
                <div class="p-5 bg-rose-500/10 border border-rose-500/20 rounded-2xl flex items-start gap-4" [@fadeIn]>
                  ...
                  <div class="flex-1">
                    ...
                    @if (state.retryable) {
                      <button (click)="onProcess(state)" class="mt-2 text-xs font-black text-violet-400 hover:text-white underline underline-offset-4">Retry</button>
                    }
                  </div>
                </div>
              }
            ...
            <div class="w-full lg:w-80 flex flex-col gap-6">
              ...
              <div class="bg-gray-900/20 rounded-2xl p-6 border border-white/5 border-dashed">
                ...
                <p class="text-xs text-gray-600 leading-relaxed">Generate video with animated audio spectrum visualization.</p>
                <div class="mt-4 space-y-2">
                  <div class="flex justify-between">
                    <span class="text-xs text-gray-600">Engine</span>
                    <span class="text-xs text-violet-400 font-bold">FFmpeg WASM</span>
                  </div>
                  <div class="flex justify-between">
...
export class VisualizerComponent implements OnDestroy {
  ...
  private service = inject(VisualizerService);
  ...
  onFileSelected(files: File[]): void {
    if (files.length > 0) {
      this.store.dispatch(VisualizerActions.loadFile({ file: files[0] }));
    }
  }

  onProcess(state: VisualizerState): void {
    ...
    const interval = setInterval(() => {
      ...
      if (progress >= 100) {
        ...
        this.store.dispatch(VisualizerActions.processingSuccess({ outputBlob: blob, outputSizeMB: blob.size / 1024 / 1024 }));
      } else {
        this.store.dispatch(VisualizerActions.updateProgress({ value: Math.round(progress) }));
      }
    }, 300);
  }

  onDownload(state: VisualizerState): void {
