...
addEventListener('message', async (event: MessageEvent<{ type: string; config: ChangeResolutionConfig & { fileBuffer: ArrayBuffer, fileName: string } }>) => {
  if (event.data.type !== 'start') return;

  const { config } = event.data;
  const { fileBuffer, fileName, width, height, maintainAspectRatio } = config;
  ...
  try {
    ...
    const outputName = `omni_res_${fileName}`;
    const args = ['-i', fileName];

    // Build scale filter
    let scaleFilter = '';
    if (maintainAspectRatio) {
      // Scale to fit within width/height, maintaining aspect ratio
