...
const initialState: UpscalerState = {
  inputFile: null, videoMeta: null, scaleFactor: 4, model: 'realesrgan',
  webGpuAvailable: false, modelDownloaded: false, modelDownloadProgress: 0,
  framesTotal: 0, framesCompleted: 0, avgFrameTimeMs: 0,
  estimatedTimeRemaining: '', processingSpeedFps: 0,
...
export const upscalerFeature = createFeature({
  ...
  reducer: createReducer(
    initialState,
    on(UpscalerActions.loadFile, (state, { file }) => ({ ...state, inputFile: file, status: 'processing' as const })),
    on(UpscalerActions.loadMetaSuccess, (state, { meta, framesTotal }) => ({ ...state, videoMeta: meta, framesTotal, status: 'idle' as const })),
    on(UpscalerActions.loadMetaFailure, (state, { errorCode, message }) => ({ ...state, status: 'error' as const, errorCode, errorMessage: message, retryable: true })),
    ...
    on(UpscalerActions.modelDownloadProgress, (state, { progress }) => ({ ...state, modelDownloadProgress: progress, status: 'downloading' as const })),
    on(UpscalerActions.startProcessing, (state) => ({ ...state, status: 'processing' as const, progress: 0, framesCompleted: 0, outputBlob: null, errorCode: null, errorMessage: null })),
    ...
    on(UpscalerActions.processingSuccess, (state, { outputBlob, outputSizeMB }) => ({ ...state, status: 'done' as const, outputBlob, outputSizeMB, progress: 100 })),
    on(UpscalerActions.processingFailure, (state, { errorCode, message, retryable }) => ({ ...state, status: 'error' as const, errorCode, errorMessage: message, retryable })),
    on(UpscalerActions.abortProcessing, (state) => ({ ...state, status: 'idle' as const, progress: 0 })),
    on(UpscalerActions.resetState, () => initialState),
  ),
});

export const {
  selectUpscalerState, selectStatus, selectProgress, selectInputFile, selectVideoMeta,
  selectScaleFactor, selectModel, selectWebGpuAvailable, selectModelDownloaded,
  selectModelDownloadProgress, selectFramesTotal, selectFramesCompleted,
  selectAvgFrameTimeMs, selectEstimatedTimeRemaining, selectProcessingSpeedFps,
