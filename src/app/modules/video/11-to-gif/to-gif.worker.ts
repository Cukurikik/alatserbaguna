...
addEventListener('message', async (event: MessageEvent<{ type: string; config: ToGifConfig & { fileBuffer: ArrayBuffer, fileName: string } }>) => {
  if (event.data.type !== 'start') return;

  const { config } = event.data;
  const { fileBuffer, fileName, fps, scale, startTime, duration, dither } = config;
  ...
  try {
    ...
    const outputName = `omni_gif_${fileName.split('.')[0]}.gif`;
    const args: string[] = [];
    ...
    const filterComplex = `fps=${fps},scale=${scale}:-1:flags=lanczos,split[s0][s1];[s0]palettegen[p];[s1][p]paletteuse=dither=${dither}`;
