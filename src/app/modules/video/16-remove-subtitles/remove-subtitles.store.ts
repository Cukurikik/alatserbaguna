...
import { RemoveSubtitlesInputSchema, RemoveSubtitlesConfig } from './remove-subtitles.schema';
import { RemoveSubtitlesService } from './remove-subtitles.service';
import { WorkerBridgeService } from '../shared/engine/worker-bridge.service';

export interface RemoveSubtitlesState {
  inputFile: File | null;
...
const initialState: RemoveSubtitlesState = {
  inputFile: null,
  videoMeta: null,
  status: 'idle',
  progress: 0,
...
export const RemoveSubtitlesStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, removeSubtitlesService = inject(RemoveSubtitlesService), workerBridge = inject(WorkerBridgeService)) => ({
    loadFile: rxMethod<{ file: File }>(
      pipe(
        ...
        switchMap(({ file }) => {
          return removeSubtitlesService.getMetadata(file).then(
            (meta) => {
              patchState(store, { 
                videoMeta: meta, 
                status: 'idle'
    ...
    startProcessing: rxMethod<void>(
      pipe(
        ...
        exhaustMap(() => {
          const state = store;
          const config: RemoveSubtitlesConfig = {
            inputFile: state.inputFile()!
          };

          const validation = RemoveSubtitlesInputSchema.safeParse(config);
          ...
          return new Promise<void>((resolve) => {
            (async () => {
              try {
                ...
                const workerFactory = () => new Worker(new URL('./remove-subtitles.worker', import.meta.url), { type: 'module' });
                
                workerBridge.process<RemoveSubtitlesConfig & { fileBuffer: ArrayBuffer, fileName: string }, Uint8Array>(workerFactory, {
    ...
    downloadOutput: () => {
      ...
      a.href = url;
      a.download = `omni_nosubs_${store.inputFile()?.name || 'video.mp4'}`;
      a.click();
