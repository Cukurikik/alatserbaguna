...
import { RemoveSubtitlesConfig } from './remove-subtitles.schema';
...
addEventListener('message', async (event: MessageEvent<{ type: string; config: RemoveSubtitlesConfig & { fileBuffer: ArrayBuffer, fileName: string } }>) => {
  if (event.data.type !== 'start') return;

  const { config } = event.data;
  const { fileBuffer, fileName } = config;
  ...
  try {
    ...
    const outputName = `omni_nosubs_${fileName}`;
    const args = ['-i', fileName];

    // Remove subtitles
    args.push('-sn');
    
    // Copy video and audio codecs to avoid re-encoding
    args.push('-c:v', 'copy');
    args.push('-c:a', 'copy');
