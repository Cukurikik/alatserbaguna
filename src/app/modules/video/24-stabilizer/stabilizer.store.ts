...
import { StabilizerInputSchema, StabilizerConfig } from './stabilizer.schema';
...
export const StabilizerStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, stabilizerService = inject(StabilizerService), workerBridge = inject(WorkerBridgeService)) => ({
    loadFile: rxMethod<{ file: File }>(
      pipe(
        ...
        switchMap(({ file }) => {
          return stabilizerService.getMetadata(file).then(
            (meta) => {
              patchState(store, { 
                videoMeta: meta, 
                status: 'idle'
    ...
    updateConfig: (config: Partial<StabilizerConfig>) => {
      ...
      pipe(
        ...
        exhaustMap(() => {
          ...
          const config: StabilizerConfig = {
            ...
            shakiness: state.shakiness(),
            accuracy: state.accuracy(),
            stepsize: state.stepsize()
          };

          const validation = StabilizerInputSchema.safeParse(config);
  ...
  // eslint-disable-next-line no-async-promise-executor
          return new Promise<void>(async (resolve) => {
            try {
              ...
              const workerFactory = () => new Worker(new URL('./stabilizer.worker', import.meta.url), { type: 'module' });
              
              workerBridge.process<StabilizerConfig & { fileBuffer: ArrayBuffer, fileName: string }, Uint8Array>(workerFactory, {
    ...
    downloadOutput: () => {
      ...
      a.href = url;
      a.download = `omni_stabilized_${store.inputFile()?.name || 'video.mp4'}`;
      a.click();
