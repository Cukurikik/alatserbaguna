...
import { VideoToDashStore } from './video-to-dash.store';
    ...
    <div class="min-h-screen bg-neutral-950 text-neutral-100 p-6">
      <div class="max-w-6xl mx-auto space-y-8">
        ...
        <div class="flex items-center space-x-4">
          ...
          <div>
            <h1 class="text-3xl font-bold tracking-tight text-white">Video to DASH</h1>
            <p class="text-neutral-400 mt-1">Convert video to MPEG-DASH (Dynamic Adaptive Streaming over HTTP).</p>
          </div>
        </div>
            ...
            <div class="bg-neutral-900 border border-neutral-800 rounded-2xl p-6 shadow-xl">
              ...
              <div class="space-y-6" [class.opacity-50]="!store.inputFile() || store.status() === 'processing'" [class.pointer-events-none]="!store.inputFile() || store.status() === 'processing'">
                ...
                <div class="space-y-3">
                  ...
                  <p class="text-[10px] text-neutral-500">Length of each .m4s segment file.</p>
                </div>
                ...
                <button 
                  ...
                  @if (store.status() === 'processing') {
                    ...
                    <span>Generating DASH...</span>
                  } @else {
                    <lucide-icon name="radio" class="w-5 h-5"></lucide-icon>
                    <span>Convert to DASH</span>
                  }
                </button>
              </div>
            </div>
              ...
              <div class="bg-neutral-900 border border-neutral-800 rounded-2xl p-6 shadow-xl">
                ...
                @if (store.status() === 'done' && store.outputBlob()) {
                  <app-export-panel
                    ...
                    [filename]="'omni_dash_' + store.inputFile()?.name + '.zip'"
...
export class VideoToDashComponent {
  store = inject(VideoToDashStore);
