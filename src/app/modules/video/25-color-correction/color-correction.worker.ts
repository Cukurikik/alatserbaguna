...
import { ColorCorrectionConfig } from './color-correction.schema';
...
addEventListener('message', async (event: MessageEvent<{ type: string; config: ColorCorrectionConfig & { fileBuffer: ArrayBuffer, fileName: string } }>) => {
  if (event.data.type !== 'start') return;

  const { config } = event.data;
  const { fileBuffer, fileName, brightness, contrast, saturation, gamma } = config;
  ...
  try {
    ...
    const outputName = `omni_color_${fileName}`;
