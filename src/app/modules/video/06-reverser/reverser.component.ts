...
const { selectReverserState } = reverserFeature;
            ...
            <div class="w-full lg:w-80 flex flex-col gap-4">
              <div class="bg-gray-800 border border-gray-700 rounded-xl p-5 text-center">
                <div class="w-20 h-20 mx-auto rounded-full bg-rose-900/40 flex items-center justify-center mb-3">
                  <svg class="w-10 h-10 text-rose-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12.066 11.2a1 1 0 000 1.6l5.334 4A1 1 0 0019 16V8a1 1 0 00-1.6-.8l-5.333 4zM4.066 11.2a1 1 0 000 1.6l5.334 4A1 1 0 0011 16V8a1 1 0 00-1.6-.8l-5.334 4z"/></svg>
                </div>
              ...
              @if (state.status === 'processing') {
                <div class="bg-gray-800 border border-gray-700 rounded-xl p-6 flex flex-col items-center">
                  <app-progress-ring [progress]="state.progress" [status]="'Reversing...'"></app-progress-ring>
                </div>
              } @else if (state.status === 'success') {
                <button (click)="onDownload()" class="w-full bg-rose-600 hover:bg-rose-500 text-white py-3 rounded-xl font-bold transition-all flex justify-center gap-2">
                ...
                <button (click)="onProcess()" class="w-full bg-gradient-to-r from-rose-500 to-red-500 hover:from-rose-400 hover:to-red-400 text-white font-bold py-3 rounded-xl shadow-[0_0_15px_rgba(244,63,94,0.4)] transition-all active:scale-95">
                  ⏪ Reverse Video
                </button>
              }
            </div>
...
export class ReverserComponent {
  ...
  onDownload(): void {
    this.state$.subscribe(s => {
      if (s.outputBlob) { const url = URL.createObjectURL(s.outputBlob); const a = Object.assign(document.createElement('a'), { href: url, download: 'omni_reversed.mp4' }); document.body.appendChild(a); a.click(); document.body.removeChild(a); setTimeout(() => URL.revokeObjectURL(url), 150); }
    }).unsubscribe();
  }

  onReset(): void { if (this.videoUrl) { URL.revokeObjectURL(this.videoUrl); this.videoUrl = null; } this.store.dispatch(ReverserActions.resetState()); }
}