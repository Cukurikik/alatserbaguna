...
export const reverserFeature = createFeature({
  ...
  reducer: createReducer(
    initialState,
    on(ReverserActions.loadFile, (state, { file }) => ({ ...state, inputFile: file, status: 'processing' as const, progress: 0 })),
    on(ReverserActions.loadMetaSuccess, (state, { meta }) => ({
      ...state, videoMeta: meta, status: 'idle' as const,
      durationWarning: meta.duration > 120,
    })),
    on(ReverserActions.loadMetaFailure, (state, { errorCode, message }) => ({
      ...state, status: 'error' as const, errorCode, errorMessage: message, retryable: true,
    })),
    on(ReverserActions.setReverseAudio, (state, { reverseAudio }) => ({ ...state, reverseAudio })),
    on(ReverserActions.startProcessing, (state) => ({ ...state, status: 'processing' as const, progress: 0, outputBlob: null, errorCode: null, errorMessage: null })),
    ...
    on(ReverserActions.processingSuccess, (state, { outputBlob, outputSizeMB }) => ({
      ...state, status: 'done' as const, outputBlob, outputSizeMB, progress: 100,
    })),
...
export const {
  selectReverserState, selectStatus, selectProgress, selectInputFile,
  selectVideoMeta, selectReverseAudio, selectDurationWarning,
