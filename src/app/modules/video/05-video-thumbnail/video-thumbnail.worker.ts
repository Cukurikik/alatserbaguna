...
addEventListener('message', async (event: MessageEvent<{ type: string; config: VideoThumbnailConfig & { fileBuffer: ArrayBuffer, fileName: string } }>) => {
  if (event.data.type !== 'start') return;

  const { config } = event.data;
  const { fileBuffer, fileName, timestamp, format, quality } = config;
