...
import { FlipRotateInputSchema, FlipRotateConfig } from './flip-rotate.schema';
...
export const FlipRotateStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, flipRotateService = inject(FlipRotateService), workerBridge = inject(WorkerBridgeService)) => ({
    loadFile: rxMethod<{ file: File }>(
      pipe(
        ...
        switchMap(({ file }) => {
          return flipRotateService.getMetadata(file).then(
            (meta) => {
              patchState(store, { videoMeta: meta, status: 'idle' });
            },
            () => {
    ...
    updateConfig: (config: Partial<FlipRotateConfig>) => {
    ...
    startProcessing: rxMethod<void>(
      pipe(
        ...
        exhaustMap(() => {
          ...
          const validation = FlipRotateInputSchema.safeParse(config);
          if (!validation.success) {
            const err = getVideoError('UNKNOWN_ERROR');
            patchState(store, { status: 'error', errorCode: err.code, errorMessage: validation.error.issues[0].message, retryable: err.retryable });
            return of(null);
          }
          ...
          return new Promise<void>((resolve) => {
            (async () => {
              try {
                ...
                const workerFactory = () => new Worker(new URL('./flip-rotate.worker', import.meta.url), { type: 'module' });
                
                workerBridge.process<FlipRotateConfig & { fileBuffer: ArrayBuffer, fileName: string }, Uint8Array>(workerFactory, {
                  ...
                  next: (msg: WorkerMessage<Uint8Array>) => {
                    if (msg.type === 'progress') {
                      ...
                      const blob = new Blob([new Uint8Array(arrayBuffer as unknown as ArrayBuffer)], { type: 'video/mp4' });
                      patchState(store, {
                        status: 'done',
                        progress: 100,
                        outputBlob: blob,
    ...
    downloadOutput: () => {
      ...
      a.href = url;
      a.download = `omni_flip_rotate_${store.inputFile()?.name || 'video.mp4'}`;
      a.click();
