...
addEventListener('message', async (event: MessageEvent<{ type: string; config: RemoveAudioConfig & { fileBuffer: ArrayBuffer, fileName: string } }>) => {
  if (event.data.type !== 'start') return;

  const { config } = event.data;
  const { fileBuffer, fileName, keepSubtitles } = config;
  ...
  try {
    ...
    const outputName = `omni_muted_${fileName}`;
