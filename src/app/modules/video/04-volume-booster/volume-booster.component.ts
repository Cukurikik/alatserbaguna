...
import { VolumeBoosterStore } from './volume-booster.store';
      ...
      <div class="max-w-6xl mx-auto space-y-8">
        ...
        <div class="flex items-center space-x-4">
          <div class="p-3 bg-emerald-500/20 rounded-xl">
            <lucide-icon name="volume-2" class="w-8 h-8 text-emerald-400"></lucide-icon>
          </div>
          <div>
            <h1 class="text-3xl font-bold tracking-tight text-white">Volume Booster</h1>
            <p class="text-neutral-400 mt-1">Increase or decrease the audio volume of your video.</p>
          </div>
        </div>
              ...
              <div class="space-y-6" [class.opacity-50]="!store.inputFile() || store.status() === 'processing'" [class.pointer-events-none]="!store.inputFile() || store.status() === 'processing'">
                ...
                <div class="space-y-3">
                  <div class="flex justify-between items-center">
                    <label for="volume-range" class="text-sm font-medium text-neutral-300">Volume Multiplier</label>
                    <span class="text-xs font-mono text-emerald-400 bg-emerald-400/10 px-2 py-1 rounded-md">
                      {{ store.volumeMultiplier() }}x
                    </span>
                  </div>
                  <input 
                    id="volume-range"
                    type="range" 
                    [ngModel]="store.volumeMultiplier()" 
                    (ngModelChange)="updateConfig('volumeMultiplier', $event)"
                    min="0" max="10" step="0.1"
                    class="w-full accent-emerald-500 bg-neutral-800 rounded-lg appearance-none h-2 cursor-pointer">
                ...
                <button 
                  ...
                  @if (store.status() === 'processing') {
                    ...
                    <lucide-icon name="volume-2" class="w-5 h-5"></lucide-icon>
              ...
              <div class="bg-neutral-900 border border-neutral-800 rounded-2xl p-6 shadow-xl">
                @if (store.status() === 'processing') {
                  <div class="flex flex-col items-center justify-center py-6 space-y-4">
                    ...
                    <p class="text-sm text-neutral-400 font-medium">Adjusting volume...</p>
                  </div>
                }
                ...
                @if (store.status() === 'done' && store.outputBlob()) {
                  <app-export-panel
                    ...
                    [filename]="'omni_volume_' + store.inputFile()?.name"
...
export class VolumeBoosterComponent {
  store = inject(VolumeBoosterStore);
  ...
  updateConfig(key: string, value: number) {
    this.store.updateConfig({ [key]: value } as Partial<VolumeBoosterConfig>);
  }
