...
import { VideoToHlsInputSchema, VideoToHlsConfig } from './video-to-hls.schema';
import { VideoToHlsService } from './video-to-hls.service';
import { WorkerBridgeService } from '../shared/engine/worker-bridge.service';

export interface VideoToHlsState {
  inputFile: File | null;
...
const initialState: VideoToHlsState = {
  inputFile: null,
  videoMeta: null,
  segmentDuration: 10,
  preset: 'fast',
...
export const VideoToHlsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, videoToHlsService = inject(VideoToHlsService), workerBridge = inject(WorkerBridgeService)) => ({
    loadFile: rxMethod<{ file: File }>(
      pipe(
        ...
        switchMap(({ file }) => {
          return videoToHlsService.getMetadata(file).then(
            (meta) => {
              patchState(store, { 
                videoMeta: meta, 
                status: 'idle'
    ...
    updateConfig: (config: Partial<VideoToHlsConfig>) => {
    ...
    startProcessing: rxMethod<void>(
      pipe(
        ...
        exhaustMap(() => {
          const state = store;
          const config: VideoToHlsConfig = {
            inputFile: state.inputFile()!,
            segmentDuration: state.segmentDuration(),
            preset: state.preset()
          };

          const validation = VideoToHlsInputSchema.safeParse(config);
          ...
          return new Promise<void>(async (resolve) => {
            try {
              ...
              const workerFactory = () => new Worker(new URL('./video-to-hls.worker', import.meta.url), { type: 'module' });
              
              workerBridge.process<VideoToHlsConfig & { fileBuffer: ArrayBuffer, fileName: string }, Uint8Array>(workerFactory, {
    ...
    downloadOutput: () => {
      ...
      a.href = url;
      a.download = `omni_hls_${store.inputFile()?.name || 'video'}.zip`;
      a.click();
