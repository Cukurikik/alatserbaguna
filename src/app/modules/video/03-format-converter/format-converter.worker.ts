...
addEventListener('message', async (event: MessageEvent<{ type: string; config: FormatConverterConfig & { fileBuffer: ArrayBuffer, fileName: string } }>) => {
  if (event.data.type !== 'start') return;

  const { config } = event.data;
  const { fileBuffer, fileName, format, preset } = config;
  ...
  try {
    ...
    const outputName = `omni_converted.${format}`;
