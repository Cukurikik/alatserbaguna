...
const { selectStabilizerState } = stabilizerFeature;
...
@Component({
            ...
            <div class="w-full lg:w-80 flex flex-col gap-4">
              <div class="bg-gray-800 border border-gray-700 rounded-xl p-5">
                <div class="flex justify-between mb-3">
                  ...
                  <span class="text-indigo-400 font-mono font-bold">{{ state.smoothing }}</span>
                </div>
                <input type="range" min="1" max="30" [value]="state.smoothing" class="w-full accent-indigo-400"
                  (input)="store.dispatch(StabilizerActions.setSmoothing({ smoothing: +$any($event.target).value }))">
              ...
              @if (state.status === 'processing') {
                <div class="bg-gray-800 border border-gray-700 rounded-xl p-6 flex flex-col items-center">
                  <app-progress-ring [progress]="state.progress" [status]="'Stabilizing...'"></app-progress-ring>
                  <p class="text-xs text-gray-400 mt-3 text-center">Pass 1: Motion detection<br>Pass 2: Transform & render</p>
                </div>
              } @else if (state.status === 'success') {
                <button (click)="onDownload()" class="w-full bg-indigo-600 hover:bg-indigo-500 text-white py-3 rounded-xl font-bold transition-all flex items-center justify-center gap-2">
  ...
  providers: [{ provide: 'STABILIZER_FEATURE', useValue: stabilizerFeature }]
})
export class StabilizerComponent {
  ...
  readonly StabilizerActions = StabilizerActions;
  ...
  onDownload(): void {
    this.state$.subscribe(s => {
      if (s.outputBlob) { const url = URL.createObjectURL(s.outputBlob); const a = Object.assign(document.createElement('a'), { href: url, download: 'omni_stabilized.mp4' }); document.body.appendChild(a); a.click(); document.body.removeChild(a); setTimeout(() => URL.revokeObjectURL(url), 150); }
    }).unsubscribe();
  }

  onReset(): void { if (this.videoUrl) { URL.revokeObjectURL(this.videoUrl); this.videoUrl = null; } this.store.dispatch(StabilizerActions.resetState()); }
}