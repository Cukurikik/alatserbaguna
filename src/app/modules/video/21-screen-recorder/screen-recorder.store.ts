...
export const screenRecorderFeature = createFeature({
  ...
  reducer: createReducer(
    ...
    on(ScreenRecorderActions.startRecording, (state) => ({ 
      ...state, 
      recording: true, 
      paused: false, 
      elapsedSeconds: 0, 
    ...
    on(ScreenRecorderActions.recordingCaptured, (state) => ({ 
      ...state, 
      status: 'processing', 
      progress: 0 
    })),
    on(ScreenRecorderActions.updateProgress, (state, { progress }) => ({ ...state, progress })),
    on(ScreenRecorderActions.processingSuccess, (state, { outputBlob, outputSizeMB }) => ({ 
      ...state, 
      status: 'success', 
      outputBlob, 
      outputSizeMB, 
    ...
    on(ScreenRecorderActions.processingFailure, (state, { errorCode, message, retryable }) => ({ 
      ...state, 
      status: 'error', 
      errorCode, 
